...
public interface FabricaDependencias {
        ...
        default EnumMap<Bonus, Integer> retornarMapBonus(){
            return mapBonus;
        }

        default int calcularSomatorioDeBonus(){
            //int n = mapBonus.size();
            int contagem = 0;
            //for (int i = 0; i < n; i++) contagem += mapBonus.get(i);
            for (EnumMap.Entry<Bonus, Integer> bonus : mapBonus.entrySet()) contagem += bonus.getValue();
            return contagem;
        }

        public IncrementadorForcaDaSenha obj(String get);

        default void AtribuirValoresMapContagemEBonus(){
            ...
            var y = mapContagem.get(Contagem.QNT_CARACTER_REPETIDO);
            ...
            mapBonus.put(Bonus.QNT_LETRA_MAIUSC, obj("IPNDCSMa").bonus);
            mapBonus.put(Bonus.QNT_LETRA_MINUSC, obj("IPNDCSMi").bonus);
            mapBonus.put(Bonus.QNT_NUMERO, IncrementadorForcaDaSenha.countNumber * 4);
            mapBonus.put(Bonus.QNT_NUMERO_OU_SIMBOLO_NO_MEIO, obj("IPNDCENMS").bonus);
            mapBonus.put(Bonus.QNT_SIMBOLO, IncrementadorForcaDaSenha.countSymbol * 6);
            mapBonus.put(Bonus.QNT_REQUERIMENTOS, obj("IPNDR").bonus);
